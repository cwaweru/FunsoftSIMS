    ...
    private void jTextField11CaretUpdate(javax.swing.event.CaretEvent evt) {//GEN-FIRST:event_jTextField11CaretUpdate
        ...
        if(this.jCheckBox2.isSelected()){
            ...
            try {
                ...
                searchRowSet.execute("SELECT (second_name||' '||first_name||' '||last_name) as name,patient_no from hp_inpatient_register where second_name||' '||first_name||' '||last_name ILIKE '"+jTextField11.getText()+"%' AND pay_mode != 'Scheme'  order by second_name");
            ...
            try {
                ...
                searchRowSet.execute("SELECT (second_name||' '||first_name||' '||last_name) as name,patient_no from hp_inpatient_register where patient_no ILIKE '"+jTextField11.getText()+"%' AND pay_mode != 'Scheme' order by second_name");
        ...
        if(this.jCheckBox2.isSelected()){
            ...
            if(jTextField11.getCaretPosition() < 3){
                ...
                if(jComboBox1.getSelectedItem().toString().equalsIgnoreCase("BBF")){
                    ...
                    try {
                        ...
                        searchRowSet.execute("select m_number,first_name||' '||middle_name||' '||last_name AS name,payroll_no from member where first_name||' '||middle_name||' '||last_name ILIKE '"+jTextField11.getText()+"%' and bbf_status ilike 'Join BBF' ORDER BY first_name||' '||middle_name||' '||last_name");
                        jSearchTable.setModel(new org.netbeans.lib.sql.models.TableModel(searchRowSet, new org.netbeans.lib.sql.models.TableModel.Column[] {
                    ...
                    try {
                        ...
                        //searchRowSet.execute("select m_number,first_name||' '||middle_name||' '||last_name AS name,department from member where first_name||' '||middle_name||' '||last_name ILIKE '"+jTextField11.getText()+"%' ORDER BY first_name||' '||middle_name||' '||last_name");//||' '||second_name||' '||last_name");
                        searchRowSet.execute("select m_number,first_name||' '||middle_name||' '||last_name AS name,payroll_no from member where first_name||' '||middle_name||' '||last_name ILIKE '"+jTextField11.getText()+"%' ORDER BY first_name||' '||middle_name||' '||last_name");
                        jSearchTable.setModel(new org.netbeans.lib.sql.models.TableModel(searchRowSet, new org.netbeans.lib.sql.models.TableModel.Column[] {
                            new org.netbeans.lib.sql.models.TableModel.Column("m_number", "member No", false),
                            new org.netbeans.lib.sql.models.TableModel.Column("name", "Name", false),
                            new org.netbeans.lib.sql.models.TableModel.Column("payroll_no", "TSC No.", false)
            ...
            if(jComboBox1.getSelectedItem().toString().equalsIgnoreCase("BBF")){
                ...
                try {
                    ...
                    //searchRowSet.execute("select m_number,first_name||' '||middle_name||' '||last_name AS name,department from member where first_name||' '||middle_name||' '||last_name ILIKE '"+jTextField11.getText()+"%' ORDER BY first_name||' '||middle_name||' '||last_name");//||' '||second_name||' '||last_name");
                    searchRowSet.execute("select m_number,first_name||' '||middle_name||' '||last_name AS name,payroll_no from member where first_name||' '||middle_name||' '||last_name ILIKE '"+jTextField11.getText()+"%' and bbf_status ilike 'Join BBF' ORDER BY payroll_no");
                    jSearchTable.setModel(new org.netbeans.lib.sql.models.TableModel(searchRowSet, new org.netbeans.lib.sql.models.TableModel.Column[] {
                        new org.netbeans.lib.sql.models.TableModel.Column("m_number", "member No", false),
                        new org.netbeans.lib.sql.models.TableModel.Column("name", "Name", false),
                        new org.netbeans.lib.sql.models.TableModel.Column("payroll_no", "TSC No.", false)
                ...
                try {
                    ...
                    //searchRowSet.execute("select m_number,first_name||' '||middle_name||' '||last_name AS name,department from member where first_name||' '||middle_name||' '||last_name ILIKE '"+jTextField11.getText()+"%' ORDER BY first_name||' '||middle_name||' '||last_name");//||' '||second_name||' '||last_name");
                    searchRowSet.execute("select m_number,first_name||' '||middle_name||' '||last_name AS name,payroll_no from member where payroll_no ILIKE '"+jTextField11.getText()+"%' ORDER BY payroll_no");
                    jSearchTable.setModel(new org.netbeans.lib.sql.models.TableModel(searchRowSet, new org.netbeans.lib.sql.models.TableModel.Column[] {
                        new org.netbeans.lib.sql.models.TableModel.Column("m_number", "member No", false),
                        new org.netbeans.lib.sql.models.TableModel.Column("name", "Name", false),
                        new org.netbeans.lib.sql.models.TableModel.Column("payroll_no", "TSC No.", false)
