...
public class CombinedNSSFPdf implements java.lang.Runnable {
    ...
    public void CombinedNSSFPdf(java.sql.Connection connDb,java.lang.String begindate,java.lang.String endate) {
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset2 = st3.executeQuery("SELECT hospital_name,nssf_no,postal_code||' '||box_no,main_telno from pb_hospitalprofile");
                            java.sql.ResultSet rset4 = st4.executeQuery("SELECT count(staff_no),sum(amount*2) from posting where (description ILIKE 'NSSF%' or description ilike 'N.s.s%') AND date BETWEEN '"+beginDate+"' AND '"+endDate+"'");
                    ...
                    try {
                        ...
                        try {
                            java.sql.Statement st3 = connectDB.createStatement();
                            java.sql.ResultSet rsetTotals = st3.executeQuery("SELECT SUM(amount*2) from posting WHERE date BETWEEN '"+beginDate+"' AND '"+endDate+"' AND (description ILIKE 'NSSF%' or description ilike 'N.s.s.%')");
                            ...
                            for (int j = 0; j < listofStaffNos.length; j++) {
                                ...
                                java.sql.ResultSet rset = st.executeQuery("select distinct id_no,first_name||' '||middle_name||' '||last_name as name,nssf_no from master_file where employee_no ILIKE '"+listofStaffNos[j]+"%' order by name");
                                
                                java.sql.ResultSet rset1 = st2.executeQuery("SELECT sum(amount*2) from posting WHERE date BETWEEN '"+beginDate+"' AND '"+endDate+"' AND staff_no = '"+listofStaffNos[j]+"'  AND (description ILIKE 'NSSF%' OR description ilike 'N.s.s.%') group by staff_no");
    ...
    public java.lang.Object[] getListofStaffNos() {
        ...
        try {
            ...
            java.sql.ResultSet rSet1 = stmt1.executeQuery("SELECT DISTINCT staff_no,staff_name from posting WHERE date BETWEEN '"+beginDate+"' AND '"+endDate+"' AND (description ILIKE 'NSSF%' or description ILIKE 'N.S.S.F%') order by staff_name");
