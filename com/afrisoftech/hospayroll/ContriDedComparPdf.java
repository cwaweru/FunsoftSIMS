...
public class ContriDedComparPdf implements java.lang.Runnable {
    ...
    public void ContriDedComparPdf(java.sql.Connection connDb, java.util.Date begindate,java.util.Date endate, java.lang.String combox,java.lang.String combox1) {
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        phrase = new Phrase(bank +  "  LIST: " , pFontHeader);
                   ...
                   /*     phrase = new Phrase("PREVIOUS MTH.",pFontHeader);
                        table.addCell(phrase);
                    ...
                    */
                        ...
                        phrase = new Phrase("BAL/DYS/HRS",pFontHeader);
                        table.addCell(phrase);
                        ...
                        for (int t = 0; t < rangeDates.length; t++) {
                            ...
                            try {
                                ...
                                com.afrisoftech.lib.DateFormatter dateFormatter = new com.afrisoftech.lib.DateFormatter(dateFormat.parse(rangeDates[t][0].toString().trim()), "MMM/yy");
                        ...
                        try {
                            ...
                            for (int j = 0; j < listofStaffNos.length; j++) {
                                ...
                                java.sql.ResultSet rset = st.executeQuery("select np.employee_no,np.first_name||' '||np.middle_name||' '||np.last_name,np.id_no,'' from master_file np where np.employee_no = '"+listofStaffNos[j]+"' order by np.employee_no");
                                java.sql.ResultSet rset1A = st3A.executeQuery("select sum(np.amount),sum(hoursdays) as days from posting np where np.description = '"+bank+"' and np.staff_no = '"+listofStaffNos[j]+"' AND date BETWEEN '"+beginDate+"' AND '"+endDate+"' and company_name ilike '"+bank1+"' group by staff_no order by np.staff_no");
                                ...
                                if (staff>0){
                                    ...
                                    while (rset1A.next()){
                                        total = total + rset1A.getDouble(1);
                                        total1 = rset1A.getDouble(1);
                                        balance = balance + rset1A.getDouble(2);
                                        double rate = rset1A.getDouble(2);
                                        ...
                                        if (rate > 0){
                                            ...
                                            phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(rset1A.getString(2)),pFontHeader1);
                                ...
                                for (int x = 0; x < rangeDates.length; x++) {
                                    java.sql.ResultSet rset1 = st3.executeQuery("select sum(np.amount) from posting np where np.description = '"+bank+"' and np.staff_no = '"+listofStaffNos[j]+"' AND date between '"+rangeDates[x][0]+"' AND '"+rangeDates[x][1]+"'");
                                    ...
                                    while (rset1.next()){
                                        ...
                                        table.addCell(phrase);
                                        columnTotals[x] = columnTotals[x] + rset1.getDouble(1);
