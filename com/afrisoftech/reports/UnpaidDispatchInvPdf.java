...
public class UnpaidDispatchInvPdf implements java.lang.Runnable {
    ...
    public void UnpaidDispatchInvPdf(java.sql.Connection connDb, java.util.Date begindate, java.util.Date endate) {
                    ...
                    try {
                        ...
                        try {
                           ...
                           // java.sql.ResultSet rsetTotals = st2.executeQuery("select sum(balance -credit_bal) from ac_debtors where dealer ilike '"+memNo+"' AND date::date <= '"+endDate+"'");
                            ...
                            phrase = new Phrase("Unpaid Dispatched Invoices", pFontHeader11);
                            table.addCell(phrase);
                            ...
                            table.getDefaultCell().setColspan(1);
                            phrase = new Phrase("Dispatch No.",pFontHeader1);
                            table.addCell(phrase);
                            ...
                            for (int j = 0; j < listofStaffNos.length; j++) {
                                ...
                                double osBalance = 0.00;
                                double osBalancebf = 0.00;
                                java.sql.ResultSet rset1 = st1.executeQuery("select dispatch_no,count(invoice_no),sum(debit),sum(balance) from ac_debtors where  payee = '"+listofStaffNos[j]+"' AND date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and sent = true group by dispatch_no");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                               // java.sql.ResultSet rsetTotals2 = st22.executeQuery("select sum(balance -credit_bal) from ac_debtors where dealer = '"+memNo+"' AND date::date <= '"+beginDate+"' and dispatch_no = '"+listofStaffNos[j]+"'");
                               ...
                               */
                                while (rset1.next()){
                                    ...
                                    DebitBal = DebitBal + rset1.getDouble(3);
                                    table.addCell(phrase);
                                    phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(rset1.getString(4)),pFontHeader);
                                    osBalancebf = osBalancebf + rset1.getDouble(4);
                                    CreditBal = CreditBal + rset1.getDouble(4);
    ...
    public java.lang.Object[] getListofStaffNos() {
        ...
        try {
            ...
            java.sql.ResultSet rSet1 = stmt1.executeQuery("SELECT DISTINCT payee FROM ac_debtors WHERE date BETWEEN '"+beginDate+"' AND '"+endDate+"' and sent = 'true' order by payee");
