...
public class MarketerAgeingPdf implements java.lang.Runnable {
    double Credit = 0.00;
   // double current = 0.00;
    //double over30 = 0.00;
   // double over60 = 0.00;
  //  double over90 = 0.00;
   // double turnOver = 0.00;
    ...
    public void MarketerAgeingPdf(java.sql.Connection connDb,java.util.Date begindate,java.util.Date endate,java.lang.String combox) {
                            ...
                            double TotalCount = 0.00;   
                               ...
                               while (rset10.next())
                                ...
                                System.out.println("Dealer Is : ["+listofAct[i]+"].");
                                 ...
                                 java.sql.PreparedStatement pset1 = connectDB.prepareStatement("select sum(balance) from ac_debtors WHERE account_no = ?   and date between  '"+beginDate+"' and '"+rangeDates[rangeDates.length - 1][0]+"' ");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                               ...
                               // java.sql.PreparedStatement pset1 = connectDB.prepareStatement("select sum(balance) from ac_debtors WHERE account_no = ?   AND date < '"+rangeDates[rangeDates.length - 1][0]+"'");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                                ...
                                java.sql.PreparedStatement pset111 = connectDB.prepareStatement("select sum(balance-credit_bal) from ac_debtors WHERE account_no = ?  AND date between '"+beginDate+"' and '"+endDate+"'");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                                ...
                                java.sql.PreparedStatement pset112 = connectDB.prepareStatement("select sum(credit_bal) from ac_debtors WHERE account_no = ?  AND date between  '"+beginDate+"' and '"+endDate+"' ");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
