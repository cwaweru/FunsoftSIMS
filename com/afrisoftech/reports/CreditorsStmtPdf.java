            ...
            ///double osBalancebf = 0.00;
                    ...
                    try {
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset1 = st1.executeQuery("select date,item,voucher_no, payee,invoice_no,credit,balance,journal_no from ac_accounts_payable where dealer = '"+memNo+"' AND date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and balance != 0 order by date");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                             java.sql.ResultSet rsetT = st41.executeQuery("select sum(balance) from ac_accounts_payable where dealer = '"+memNo+"' AND date::date < '"+beginDate+"'");
                            ...
                            java.sql.ResultSet rsetTotals = st2.executeQuery("select sum(credit),sum(balance) from ac_accounts_payable where dealer = '"+memNo+"' AND date::date BETWEEN '"+beginDate+"' AND '"+endDate+"'");
                            //                       if(docPdf.getPageNumber() == 1) {
