...
public class TransByAccountSummPdf implements java.lang.Runnable {
    ...
    public void TransByAccountSummPdf(java.sql.Connection connDb, java.util.Date begindate, java.util.Date endate, java.lang.String combox) {
    ...
    public void generatePdf() {
      ...
      */
                    ...
                    com.lowagie.text.HeaderFooter footer = new com.lowagie.text.HeaderFooter(new Phrase("Transaction By Account(Summary) - Page: ",pFontHeader), true);// FontFactory.getFont(com.lowagie.text.FontFactory.HELVETICA, 12, Font.BOLDITALIC,java.awt.Color.blue));
                    ...
                    try {
                        ...
                        int headerwidths[] = {12,10,20,15,13,13};
                        ...
                        try {
                            ...
                            phrase = new Phrase(bank + "  "+    Activity+" :  Transactions Report(Summary)"    +"   Period : From "  +dateFormat.format(endDate11)+" To "+dateFormat.format(endDate1), pFontHeader);
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset = st.executeQuery("select date,sum(debit),sum(credit) from transaction_list_view WHERE date BETWEEN '"+beginDate+"' AND '"+endDate+"' AND activity_code = '"+bank+"' GROUP BY date ORDER BY date ASC");// tn,debit_note db WHERE tn.policy_no != '' and tn.policy_no = db.policy_no GROUP BY tn.policy_no,db.policy_class");
                            
                            // java.sql.ResultSet rset = st.executeQuery("select date,initcap(dealer),initcap(payee),initcap(description),voucher_no,cheque_no,debit,credit from ac_ledger WHERE date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' AND activity_code = '"+bank+"' order by date");// tn,debit_note db WHERE tn.policy_no != '' and tn.policy_no = db.policy_no GROUP BY tn.policy_no,db.policy_class");
                            
                            // java.sql.ResultSet rsetTotals = st2.executeQuery("SELECT sum(debit),SUM(credit) from transaction_list_view WHERE date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' AND activity_code = '"+bank+"'");
                            ...
                            while (rset.next()) {
                                ...
                                phrase = new  Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(rset.getString(2)), pFontHeader);
