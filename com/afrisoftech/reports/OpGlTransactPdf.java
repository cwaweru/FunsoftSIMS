...
public class OpGlTransactPdf implements java.lang.Runnable {
    ...
    public void OpGlTransactPdf(java.sql.Connection connDb,java.util.Date begindate, java.util.Date endate) {
                       ...
                       // Phrase phrase = new Phrase("Patients List As At:" +endDate, pFontHeader);
                        ...
                        try {
                            ...
                            for (int i = 0; i < listofAct.length; i++) {
                                ...
                                java.sql.PreparedStatement psetc = connectDB.prepareStatement("select service,sum(amount) from hp_patient_billing  where trans_date BETWEEN '"+beginDate+"' AND '"+endDate+"' and  revenue_code = ? group by service UNION ALL SELECT description,sum(amount) from hp_pharmacy where date_prescribed between '"+beginDate+"' AND '"+endDate+"' and patient_no NOT LIKE '0%' GROUP BY description");
                               psetc.setString(1,listofAct[i].toString());
                                java.sql.ResultSet rset = psetc.executeQuery();
                                
                              //  java.sql.PreparedStatement pset1 = connectDB.prepareStatement("select description,sum(amount) from hp_patient_billing  trans_date BETWEEN '"+beginDate+"' AND '"+endDate+"' group by description UNION ALL SELECT description,sum(amount) from hp_pharmacy where date_prescribed between '"+beginDate+"' AND '"+endDate+"' and patient_no NOT LIKE '0%' GROUP BY description");
                              //  pset1.setString(1,listofAct[i].toString());
                               //  pset1.setString(2,listofAct[i].toString());
                               // java.sql.ResultSet rsetTotals = pset1.executeQuery();
                                
                               
                                
                               // java.sql.ResultSet rset = st.executeQuery("select date::date,gl_code,description,transaction_no,sum(debit),sum(credit) from ac_general_ledger_view where gl_code = '"+listofAct[i]+"' AND description IS NOT NULL AND transaction_no IS NOT NULL AND transaction_no !='' AND date BETWEEN '"+beginDate+"' AND '"+endDate+"' group by date::date,gl_code,description,transaction_no");// tn,debit_note db WHERE tn.policy_no != '' and tn.policy_no = db.policy_no GROUP BY tn.policy_no,db.policy_class");
                               // System.out.println("Select A done ...");
                               // java.sql.ResultSet rsetTotals = st2.executeQuery("SELECT SUM(tl.debit),SUM(tl.credit),ac.activity from ac_general_ledger_view tl,pb_activity ac where tl.gl_code ='"+listofAct[i].toString()+"' and ac.code = '"+listofAct[i].toString()+"' AND tl.date BETWEEN '"+beginDate+"' AND '"+endDate+"' GROUP BY ac.activity");
                               // System.out.println("Select B done ...");
                                rsetTotals1 = st3.executeQuery("select service,sum(amount) from hp_patient_billing  where trans_date BETWEEN '"+beginDate+"' AND '"+endDate+"' group by service UNION ALL SELECT description,sum(amount) from hp_pharmacy where date_prescribed between '"+beginDate+"' AND '"+endDate+"' and patient_no NOT LIKE '0%' GROUP BY description");
    ...
    public java.lang.Object[] getListofActivities() {
        ...
        try {
            ...
            java.sql.ResultSet rSet1 = stmt1.executeQuery("select UPPER(revenue_code) from hp_patient_billing where trans_date between '"+beginDate+"' AND '"+endDate+"' UNION ALL SELECT 'PHARMACY' from hp_pharmacy where date_prescribed between '"+beginDate+"' AND '"+endDate+"' and patient_no NOT LIKE '0%'");// tn,debit_note db WHERE tn.policy_no != '' and tn.policy_no = db.policy_no GROUP BY tn.policy_no,db.policy_class");
