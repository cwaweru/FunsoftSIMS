...
public class PayerStatementDetailPdf implements java.lang.Runnable {
    ...
    public void PayerStatementDetailPdf(java.sql.Connection connDb, java.util.Date begindate, java.util.Date endate,java.lang.String combox) {
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        int headerwidths[] = {12,11,25,10,12,12,10,10,15};
                        ...
                        try {
                            ...
                            phrase = new Phrase(memNo+ " Statement Of Account", pFontHeader11);
                            table.addCell(phrase);
                            ...
                            for (int j = 0; j < listofStaffNos.length; j++) {
                                ...
                                java.sql.ResultSet rset1 = st1.executeQuery("select date,admission_no||''||receipt_no, item,invoice_no,debit,credit,credit_bal,balance from ac_debtors where dealer ilike '"+memNo+"' and account_no = '"+listofStaffNos[j]+"' and ((balance)<>0 or (CREDIT_BAL)<>0) AND date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' order by date");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                                java.sql.ResultSet rsetTotals2 = st22.executeQuery("select sum(balance -credit_bal) from ac_debtors where dealer ilike '"+memNo+"%' AND date::date <='"+beginDate+"' and account_no ilike '"+listofStaffNos[j]+"'");
                                 java.sql.ResultSet rsetTotals21 = st221.executeQuery("select scheme_name from ac_schemes where account_no ilike '"+listofStaffNos[j]+"'");
                             ...
                             table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                ...
                                table.addCell(phrase);
                                  while (rsetTotals21.next()){
                                     table.getDefaultCell().setColspan(3);
                                phrase = new Phrase(dbObject.getDBObject(rsetTotals21.getObject(1), "-"), pFontHeader1);
                              ...
                              //  table.getDefaultCell().setBorderColor(java.awt.Color.BLACK);
                               // table.getDefaultCell().setBorderWidth(Rectangle.TOP | Rectangle.BOTTOM);
                                ...
                                while (rset1.next()){
                                    ...
                                    DebitBal = DebitBal + rset1.getDouble(7);
                                    table.addCell(phrase);
                                    ...
                                    CreditBal = CreditBal + rset1.getDouble(8);
                                    ...
                                    phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(java.lang.String.valueOf(osBalancebf - osBalance1)),pFontHeader);
                                      Total = Total +(osBalancebf - osBalance1);
                               ...
                               table.getDefaultCell().setBorder(Rectangle.BOTTOM);
                                ...
                                phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(java.lang.String.valueOf(osBalancebf - osBalance1)),pFontHeadert);
                                 table.addCell(phrase);
                                
                                Total = osBalancebf - osBalance1;
                                //phrase = new Phrase(" ");
    ...
    public java.lang.Object[] getListofStaffNos() {
        ...
        try {
           ...
           // java.sql.ResultSet rSet1 = stmt1.executeQuery("SELECT DISTINCT payee FROM ac_debtors WHERE  dealer ilike '"+memNo+"%' order by payee");
            java.sql.ResultSet rSet1 = stmt1.executeQuery("SELECT DISTINCT account_no FROM ac_debtors WHERE  dealer  ilike '"+memNo+"%' and ((balance)<>0 or (CREDIT_BAL)<>0) order by account_no");
