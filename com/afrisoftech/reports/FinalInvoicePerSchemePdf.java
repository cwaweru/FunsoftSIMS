...
public class FinalInvoicePerSchemePdf implements java.lang.Runnable {
    ...
    public void FinalInvoicePerSchemePdf(java.sql.Connection connDb, java.util.Date begindate, java.util.Date endate,java.lang.String combox,java.lang.String combox1) {
                        ...
                        for (int j = 0; j < listofStaffNos.length; j++) {
                            ...
                            try {
                                ...
                                java.sql.Statement st5 = connectDB.createStatement();
                                 java.sql.ResultSet rset31 = st31.executeQuery("select sp.scheme_name,sp.postal_code||' '||sp.address,sp.tel_main||' '||sp.other_tel,sp.main_fax from ac_schemes sp where sp.scheme_name = '"+memNo+"'");
                            //  java.sql.ResultSet rset31 = st31.executeQuery("select pc.description,sp.postal_code||' '||sp.address,sp.tel_main||' '||sp.other_tel,sp.main_fax from ac_scheme_providers sp,hp_patient_register pc where pc.payer = sp.scheme_manager and pc.patient_no = '"+listofStaffNos[j]+"'");
                                java.sql.ResultSet rset4 = st4.executeQuery("select distinct dealer from ac_debtors where payee = '"+memNo+"' and date BETWEEN '"+beginDate+"' AND '"+endDate+"'");
                             ...
                             //   java.sql.ResultSet rset3 = st3.executeQuery("select hospital_name,postal_code||' '||box_no||' '||town,main_telno||' '||other_telno,initcap(street),main_faxno,email,website,room_no from pb_hospitalprofile");
                               // java.sql.ResultSet rset4 = st4.executeQuery("select sp.scheme_manager,sp.postal_code||' '||sp.address,sp.tel_main||' '||sp.other_tel,sp.main_fax from ac_scheme_providers sp,ac_debtors pc where pc.dealer = sp.scheme_manager and pc.payee = '"+memNo+"'");
                              ...
                              java.sql.ResultSet rset5 = st5.executeQuery("select account_no,member_name from hp_patient_register where patient_no = '"+listofStaffNos[j]+"'");
                              ...
                              //  java.sql.ResultSet rset5 = st5.executeQuery("select CASE WHEN (pc.scheme_staff_no IS NULL) THEN sm.staff_no ELSE pc.scheme_staff_no END AS number,CASE WHEN (sh.staff_name IS NULL) THEN sm.staff_name ELSE sh.staff_name END AS name from hp_schemestaff sh, hp_patient_card pc,scheme_members_view sm where pc.patient_no = '"+listofStaffNos[j]+"' AND pc.scheme_staff_no = sm.staff_no and pc.scheme = sh.sheme_name and sm.staff_no = pc.scheme_staff_no");
                                java.sql.ResultSet rset1 = st1.executeQuery("select pc.date::date,initcap(pc.service) as service,pc.dosage,pc.reference,pc.credit from hp_patient_card pc,ac_debtors db where pc.patient_no = '"+listofStaffNos[j]+"' AND pc.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' AND pc.paid = true and db.payee = '"+memNo+"' and db.admission_no = pc.patient_no and pc.credit > 0 and pc.reference IS NOT NULL order by pc.date::date");
                                java.sql.ResultSet rsetTotals = st2.executeQuery("select sum(db.debit) from hp_patient_card pc ,ac_debtors db where db.admission_no = '"+listofStaffNos[j]+"' AND db.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and db.payee = '"+memNo+"'");
                                ...
                                java.sql.ResultSet rset12 = st12.executeQuery("select date from ac_debtors where admission_no = '"+listofStaffNos[j]+"' and date BETWEEN '"+beginDate+"' AND '"+endDate+"' and payee = '"+memNo+"'");
                                java.sql.ResultSet rset11 = st11.executeQuery("select invoice_no from ac_debtors where admission_no = '"+listofStaffNos[j]+"' AND date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and payee = '"+memNo+"' and invoice_no IS NOT NULL");
                         ...
                         /*       while (rset3.next())
                                ...
                                */
                                ...
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                  phrase = new Phrase("Payer Name: -", pFontHeader1);
    ...
    public java.lang.Object[] getListofStaffNos() {
        ...
        try {
            ...
            //    java.sql.Connection connDB = java.sql.DriverManager.getConnection("jdbc:postgresql://localhost:5432/sako","postgres","pilsiner");
              java.sql.PreparedStatement pSet1 = connectDB.prepareStatement("SELECT admission_no FROM ac_debtors WHERE date  BETWEEN ? AND ? AND payee = ? order by admission_no");
