...
public class SchemeStaffPdf implements java.lang.Runnable {
    ...
    public void SchemeStaffPdf(java.sql.Connection connDb, java.util.Date begindate, java.util.Date endate,java.lang.String combox1,java.lang.String combox) {
                  ...
                  //  docPdf.open();
                    ...
                    try {
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset1 = st1.executeQuery("select staff_no,staff_name,birth_date from hp_schemestaff where account_no = '"+memNo+"' AND date_set::date BETWEEN '"+beginDate+"' AND '"+endDate+"' order by date_set");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                            java.sql.ResultSet rset11 = st11.executeQuery("select count(staff_no) from hp_schemestaff where account_no = '"+memNo+"' AND date_set::date BETWEEN '"+beginDate+"' AND '"+endDate+"'");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                           ...
                           // java.sql.ResultSet rsetTotals = st2.executeQuery("select sum(debit),sum(credit) from ac_debtors where account_no = '"+memNo+"' AND date::date BETWEEN '"+beginDate+"' AND '"+endDate+"'");
                            ...
                            phrase = new Phrase("Scheme Staff", pFontHeader11);
                            table.addCell(phrase);
