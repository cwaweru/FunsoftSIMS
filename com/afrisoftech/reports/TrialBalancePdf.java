                    ...
                    try {
                       ...
                       // Phrase phrase = new Phrase("Patients List As At:" +endDate, pFontHeader);
                        ...
                        try {
                           ...
                           // java.sql.Statement st2 = connectDB.createStatement();
                          //  java.sql.ResultSet rset = st.executeQuery("SELECT case when (tb.gl_code IS NULL) THEN '-' ELSE tb.gl_code END as gl_code,initcap(ac.activity),SUM(tb.debit - tb.credit) from trial_balance_view tb,pb_activity ac WHERE tb.date BETWEEN '"+beginDate+"' AND '"+endDate+"'");// AND tb.transaction_no IS NOT NULL AND tb.gl_code !='' AND ac.code = tb.gl_code  group by tb.gl_code,ac.activity order by tb.gl_code");
                            java.sql.ResultSet rset = st.executeQuery("SELECT gl_code,SUM(debit - credit) from trial_balance_view WHERE date BETWEEN '"+beginDate+"' AND '"+endDate+"' group by gl_code order by gl_code");                            
