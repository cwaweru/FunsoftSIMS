...
public class UnApprovedInvPerScmPdf implements java.lang.Runnable {
    ...
    public void  UnApprovedInvPerScmPdf(java.sql.Connection connDb, java.util.Date begindate, java.util.Date endate,java.lang.String combox,java.lang.String combox1) {
                    ...
                    try {
                      ...
                      //  }
                        ...
                        //   java.sql.ResultSet rset1 = st1.executeQuery("select ac.date,pc.scheme_staff_no, ac.payee,ac.reference_no,sum(ac.debit),ac.admission_no from hp_patient_card pc,ac_debtors ac where pc.patient_no = ac.admission_no and ac.dealer = '"+memNo+"' and ac.dealer = pc.scheme AND ac.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and ac.debit > 0 group by ac.date,pc.scheme_staff_no, ac.payee,ac.reference_no,ac.admission_no");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                           java.sql.ResultSet rset1 = st1.executeQuery("select date,admission_no, item,invoice_no,debit,balance from ac_debtors where account_no = '"+memNo1+"' AND date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and balance > 0 AND sent = false order by date");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                          java.sql.ResultSet rsetTotals2 = st22.executeQuery("select sum(debit),sum(balance) from ac_debtors where account_no = '"+memNo1+"' AND date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' AND sent = false");
                             java.sql.ResultSet rsetTotals = st2.executeQuery("select sum(debit),sum(balance) from ac_debtors where account_no = '"+memNo1+"' AND date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' AND sent = false");
      //                       if(docPdf.getPageNumber() == 1) {
                       ...
                       //     table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                         ...
                         */
                            ...
                            table.getDefaultCell().setColspan(7);
                              table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_CENTER);
                            phrase = new Phrase("Unapproved Invoices", pFontHeader11);
                            table.addCell(phrase);
