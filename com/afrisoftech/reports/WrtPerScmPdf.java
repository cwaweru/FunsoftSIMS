...
public class WrtPerScmPdf implements java.lang.Runnable {
    ...
    public void WrtPerScmPdf(java.sql.Connection connDb, java.util.Date begindate, java.util.Date endate,java.lang.String combox1,java.lang.String combox) {
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        int headerwidths[] = {12,12,12,25,13,13,13};
                       ...
                       table.getDefaultCell().setBorder(Rectangle.BOTTOM);
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset1 = st1.executeQuery("select db.date,db.wrt_date,db.admission_no,db.item,db.invoice_no,db.debit, db.wrt_amt from ac_debtors db where db.account_no = '"+memNo+"' AND db.wrt_date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' order by db.wrt_date");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                          ...
                          //  java.sql.ResultSet rsetTotals = st2.executeQuery("select sum(amount),sum(amount_alloc) from db_allocation where acc_no = '"+memNo+"' AND date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and acc_no = '"+memNo+"'");
                            ...
                            phrase = new Phrase("WriteOff Report", pFontHeader11);
                            table.addCell(phrase);
                          ...
                          //  table.getDefaultCell().setBorderColor(java.awt.Color.BLACK);
                            ...
                            table.addCell(phrase);
                            phrase = new Phrase("W/O Date",pFontHeader1);
                            table.addCell(phrase);
                            table.getDefaultCell().setColspan(1);
                            ...
                            table.addCell(phrase);
                             phrase = new Phrase("W/0 Amt.",pFontHeader1);
                            table.addCell(phrase);
