...
public class MarketerTurnoverAgedPdf implements java.lang.Runnable {
    double Credit = 0.00;
   // double current = 0.00;
    //double over30 = 0.00;
   // double over60 = 0.00;
  //  double over90 = 0.00;
   // double turnOver = 0.00;
    ...
    public void MarketerTurnoverAgedPdf(java.sql.Connection connDb,java.util.Date begindate,java.util.Date endate,java.lang.String combox) {
                ...
                try {
                   ...
                   */
                     ...
                     */
                        ...
                        Phrase phrase = new Phrase(""+Debtor+" Schemes (Aged)");
                     ...
                     try {
                        ...
                        //      table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                        phrase = new Phrase("Credits",pFontHeader);
                        table.addCell(phrase);
                            ...
                            double TotalCount = 0.00;   
                               ...
                               while (rset10.next())
                                ...
                                System.out.println("Dealer Is : ["+listofAct[i]+"].");
                                 ...
                                 java.sql.PreparedStatement pset1 = connectDB.prepareStatement("select sum(debit) from ac_debtors WHERE account_no = ?   and date between  '"+beginDate+"' and '"+rangeDates[rangeDates.length - 1][0]+"' ");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                               ...
                               // java.sql.PreparedStatement pset1 = connectDB.prepareStatement("select sum(balance) from ac_debtors WHERE account_no = ?   AND date < '"+rangeDates[rangeDates.length - 1][0]+"'");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                                ...
                                java.sql.PreparedStatement pset111 = connectDB.prepareStatement("select sum(debit) from ac_debtors WHERE account_no = ?  AND date between '"+beginDate+"' and '"+endDate+"'");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                                ...
                                java.sql.PreparedStatement pset112 = connectDB.prepareStatement("select sum(credit) from ac_debtors WHERE account_no = ?  AND date between  '"+beginDate+"' and '"+endDate+"' ");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                                ...
                                for (int t = 0; t < ageingDates.length; t++) {
                                    ...
                                    java.sql.PreparedStatement pset = connectDB.prepareStatement("select sum(debit) from ac_debtors WHERE account_no = ?   AND date between '"+rangeDates[t][1]+"' AND '"+rangeDates[t][0]+"'");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
    ...
    public java.lang.Object[] getListofActivities() {
        ...
        try {
           ...
           // java.sql.PreparedStatement pSet1 = connectDB.prepareStatement("SELECT DISTINCT scheme_manager FROM ac_scheme_providers  where scheme_manager is not null and scheme_manager !='' order by scheme_manager");
             java.sql.PreparedStatement pSet1 = connectDB.prepareStatement("SELECT DISTINCT ac.account_no,sum(db.balance -db.credit_bal) as balance FROM  ac_debtors db ,ac_schemes ac WHERE ac.marketing_contact  ILIKE '"+Debtor+"' and ac.account_no = db.account_no and balance != 0 group by ac.account_no order by ac.account_no");
