...
public class DispPerPayerSummPdf implements java.lang.Runnable {
    ...
    public void DispPerPayerSummPdf(java.sql.Connection connDb, java.util.Date begindate, java.util.Date endate,java.lang.String combox1) {
                      ...
                      //  }
                       ...
                       // table.getDefaultCell().setBorderColor(java.awt.Color.WHITE);
                        ...
                        //   java.sql.ResultSet rset1 = st1.executeQuery("select ac.date,pc.scheme_staff_no, ac.payee,ac.reference_no,sum(ac.debit),ac.admission_no from hp_patient_card pc,ac_debtors ac where pc.patient_no = ac.admission_no and ac.dealer = '"+memNo+"' and ac.dealer = pc.scheme AND ac.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and ac.debit > 0 group by ac.date,pc.scheme_staff_no, ac.payee,ac.reference_no,ac.admission_no");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                           java.sql.ResultSet rset1 = st1.executeQuery("select dispatch_date,dispatch_no,count(invoice_no) ,payee,sum(debit),sum(balance) from ac_debtors where dealer = '"+memNo1+"' AND dispatch_date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and dispatch_no is not null group by dispatch_date,dispatch_no ,payee order by dispatch_no");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                          java.sql.ResultSet rsetTotals2 = st22.executeQuery("select sum(balance-credit_bal) from ac_debtors where dealer = '"+memNo1+"' AND dispatch_date < '"+beginDate+"'");
                            java.sql.ResultSet rset412 = st412.executeQuery("select count(invoice_no) from ac_debtors where dealer = '"+memNo1+"' AND dispatch_date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and balance > 0 and dispatch_no is not null");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                           ...
                           // table.getDefaultCell().setBorderWidth(Rectangle.TOP | Rectangle.BOTTOM);
                            ...
                            phrase = new Phrase("Dispatch Schedule List:Per Payer", pFontHeader11);
                            table.addCell(phrase);
