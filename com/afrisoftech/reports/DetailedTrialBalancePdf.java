...
public class DetailedTrialBalancePdf implements java.lang.Runnable {
    ...
    public void DetailedTrialBalancePdf(java.sql.Connection connDb,java.util.Date begindate, java.util.Date endate) {
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            phrase = new Phrase("Trial Balance :     Period : From "  +dateFormat.format(endDate11)+" To "+dateFormat.format(endDate1), pFontHeader);
                        ...
                        try {
                            ...
                            //  java.sql.ResultSet rset = st.executeQuery("SELECT tb.gl_code,CASE WHEN (SUM(tb.debit-credit) > 0) THEN sum(debit-credit) ELSE 0.00 END AS debit,CASE WHEN (sum(tb.credit-tb.debit) > 0) THEN SUM(tb.credit-tb.debit) ELSE 0.00 END AS credit from detailed_trial_balance_view tb WHERE tb.date BETWEEN '"+beginDate+"' AND '"+endDate+"' group by tb.gl_code order by tb.gl_code");
                            java.sql.ResultSet rset = st.executeQuery("SELECT tb.code,sum(tb.ytd_debit) AS debit,sum(tb.ytd_credit) AS credit from pb_activity tb group by tb.code order by tb.code");
                            ...
                            while (rset.next()) {
                                ...
                                phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(java.lang.String.valueOf(credit2)), pFontHeader1);
