...
public class DispatchSchPdf implements java.lang.Runnable {
    ...
    public void DispatchSchPdf(java.sql.Connection connDb,java.lang.String combox,java.lang.String combox1) {
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset1 = st1.executeQuery("select date,journal_no, initcap(item),invoice_no ||' '||receipt_no,debit,debit,null as boolean from ac_debtors where dispatch_no = '"+memNo+"' and debit > 0  order by date");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                            ...
                            while (rset41.next()){
                                ...
                                table.addCell(phrase);
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase("Scheme No : -", pFontHeader1);
                                table.addCell(phrase);
                            }
                            while (rset5.next()){
                                table.getDefaultCell().setColspan(4);
                                ...
                                table.addCell(phrase);
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase("Scheme Acc/No. : -", pFontHeader1);
                                table.addCell(phrase);
                            }
