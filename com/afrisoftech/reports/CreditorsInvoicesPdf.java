...
public class CreditorsInvoicesPdf implements java.lang.Runnable {
    ...
    public void CreditorsPdf(java.sql.Connection connDb,java.util.Date begindate, java.util.Date endate ) {
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        int headerwidths[] = {20,14,40,35,12,12,12};
                        ...
                        Phrase phrase = new Phrase("");
                         try {
                                ...
                                phrase = new Phrase("Creditors Invoices List :     Period : From "  +dateFormat.format(endDate11)+" To "+dateFormat.format(endDate1), pFontHeader);
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset = st.executeQuery("select invoice_no,date,dealer,sum(credit),item,sum(debit),sum(credit-debit) from ac_accounts_payable WHERE date BETWEEN '"+beginDate+"' AND '"+endDate+"' and (transaction_type ILIKE 'Stock Delivery' OR transaction_type ILIKE 'Raise Invoice' OR transaction_type ILIKE 'INV') GROUP BY invoice_no,date,dealer,item ORDER BY date");
                            java.sql.ResultSet rsetTotals = st2.executeQuery("SELECT SUM(credit),sum(debit),sum(credit-debit) from ac_accounts_payable WHERE date BETWEEN '"+beginDate+"' AND '"+endDate+"' and (transaction_type ILIKE 'Stock Delivery' OR transaction_type ILIKE 'Raise Invoice' OR transaction_type ILIKE 'INV')");
  ...
  */
                                ...
                                table.getDefaultCell().setColspan(1);
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase(dbObject.getDBObject(rset.getObject(5), "-"), pFontHeader);
