                  ...
                  */
                    ...
                    try {
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset1 = st1.executeQuery("select date,admission_no||' '||cheque_no, upper(item),invoice_no ||' '||receipt_no,debit,credit,credit_bal,balance from ac_debtors where account_no = '"+memNo+"' AND date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' order by date");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                              java.sql.ResultSet rsetTotals2 = st22.executeQuery("select sum(balance -credit_bal) from ac_debtors where account_no = '"+memNo+"' AND date::date < '"+beginDate+"'");
                          // java.sql.ResultSet rsetTotalsD = st22D.executeQuery("select sum(credit-debit) from ac_ledger where payee = '"+memNo+"'");
                            ...
                            table.addCell(phrase);
                            //table
                           /*  while (rsetTotalsD.next())
                                table.getDefaultCell().setColspan(5);
                                
                                table.getDefaultCell().setBorderColor(java.awt.Color.WHITE);
                                
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase("Account Deposit", pFontHeader1);
                                 table.addCell(phrase);
                                 table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_RIGHT);
                                table.getDefaultCell().setColspan(4);
                            phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(rsetTotalsD.getString(1)),pFontHeader1);
