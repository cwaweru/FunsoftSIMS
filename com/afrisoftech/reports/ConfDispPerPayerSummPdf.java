...
public class ConfDispPerPayerSummPdf implements java.lang.Runnable {
    ...
    public void ConfDispPerPayerSummPdf(java.sql.Connection connDb, java.util.Date begindate, java.util.Date endate,java.lang.String combox1) {
                    ...
                    try {
                      ...
                      //  com.lowagie.text.Table table = new com.lowagie.text.Table(7);
                       ...
                       // table.endHeaders();
                        ...
                        int headerwidths[] = {12,8,8,20,13,15,15};
                       ...
                       // table.getDefaultCell().setBorderColor(java.awt.Color.WHITE);
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset3 = st3.executeQuery("select hospital_name,postal_code||' '||box_no||' '||town,main_telno||' '||other_telno,initcap(street),main_faxno,email,website,room_no from pb_hospitalprofile");
                              java.sql.ResultSet rset41 = st41.executeQuery("select scheme_manager from ac_scheme_providers where scheme_manager = '"+memNo1+"'");
                        //   java.sql.ResultSet rset1 = st1.executeQuery("select ac.date,pc.scheme_staff_no, ac.payee,ac.reference_no,sum(ac.debit),ac.admission_no from hp_patient_card pc,ac_debtors ac where pc.patient_no = ac.admission_no and ac.dealer = '"+memNo+"' and ac.dealer = pc.scheme AND ac.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and ac.debit > 0 group by ac.date,pc.scheme_staff_no, ac.payee,ac.reference_no,ac.admission_no");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                           java.sql.ResultSet rset1 = st1.executeQuery("select prov_date,dispatch_no,count(invoice_no) ,payee,sum(debit),sum(debit) from ac_debtors where dealer = '"+memNo1+"' AND prov_date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and dispatch_no is not null and approved = true group by prov_date,dispatch_no ,payee order by dispatch_no");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                          java.sql.ResultSet rsetTotals2 = st22.executeQuery("select sum(debit) from ac_debtors where dealer = '"+memNo1+"' AND prov_date < '"+beginDate+"'");
                            java.sql.ResultSet rset412 = st412.executeQuery("select count(invoice_no) from ac_debtors where dealer = '"+memNo1+"' AND prov_date BETWEEN '"+beginDate+"' AND '"+endDate+"'  and approved = true and dispatch_no is not null");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                        
       
                            table.getDefaultCell().setColspan(7);
                              table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_CENTER);
                            phrase = new Phrase("Confirmed Dispatch :Per Payer", pFontHeader11);
                            table.addCell(phrase);
                        ...
                        table.addCell(phrase);
                           ...
                           // table.addCell(phrase);
                            ...
                            table.getDefaultCell().setColspan(1);
                            phrase = new Phrase("Invoice Count",pFontHeader1);
                            table.addCell(phrase);
                             table.getDefaultCell().setColspan(1);
                            ...
                            while (rset1.next()){
                                ...
                                osDebit = osDebit + rset1.getDouble(5);
                          ...
                          //  while (rsetTotals.next()) {
                            ...
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                            phrase = new Phrase(dbObject.getDBObject(rset412.getObject(1), "-"), pFontHeader1);
                            table.addCell(phrase);
                            phrase = new Phrase(" ", pFontHeader1);
                                
                                table.addCell(phrase);
                                  table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_RIGHT);
                                phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(java.lang.String.valueOf(osDebit)),pFontHeader1);
