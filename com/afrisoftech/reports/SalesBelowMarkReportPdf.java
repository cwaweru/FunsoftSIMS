...
public class SalesBelowMarkReportPdf implements java.lang.Runnable {
    ...
    public void SalesBelowMarkReportPdf(java.sql.Connection connDb,java.util.Date begindate, java.util.Date endate) {
            ...
            double osBalance3 = 0.00;
                        try {
                          ...
                          //  java.sql.Statement pSet1 = connectDB.createStatement();
                            ...
                            //java.sql.ResultSet rsetTotals = st.executeQuery("select sum((ph.quantity * st.transfer_price)::numeric(10,2)), sum(amount)::numeric(10,2),sum(amount - ph.quantity * st.transfer_price)::numeric(10,2) from hp_pharmacy ph,st_stock_prices st WHERE (st.department ilike 'PHAr%' OR st.department ilike 'IP Pharmacy%') AND ph.date_prescribed BETWEEN '"+beginDate+"' AND '"+endDate+"' AND ph.description = st.product");
                            ...
                            for (int i = 0; i < listofAct.length; i++) {
                               ...
                               // java.sql.PreparedStatement st1 = connectDB.prepareStatement("select ph.description,sum(ph.quantity),avg(st.transfer_price)::numeric(10,2),sum((ph.quantity * st.transfer_price)::numeric(10,2)), avg(st.selling_price)::numeric(10,2),sum(amount)::numeric(10,2),sum(amount - ph.quantity * st.transfer_price)::numeric(10,2) from hp_pharmacy ph,st_stock_prices st WHERE (st.department ilike 'PHAr%' OR st.department ilike 'IP Pharmacy%') and ph.description = ? AND ph.date_prescribed BETWEEN '"+beginDate+"' AND '"+endDate+"' AND ph.description = st.product GROUP BY ph.description");
    ...
    public java.lang.Object[] getListofActivities() {
        ...
        try {
            ...
            java.sql.ResultSet rSet1 = stmt1.executeQuery("SELECT DISTINCT description,mark FROM markup_view  where mark != 1.33 AND date_prescribed BETWEEN '"+beginDate+"' AND '"+endDate+"' order by mark asc ,description");
