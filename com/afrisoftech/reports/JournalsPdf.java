...
public class JournalsPdf implements java.lang.Runnable {
    ...
    public void JournalsPdf(java.sql.Connection connDb,java.util.Date begindate, java.util.Date endate) {
                    ...
                    try {
                        ...
                        Phrase phrase = new Phrase("", pFontHeader);
                            ...
                            try {
                                ...
                                phrase = new Phrase("Journals : Period : From "  +dateFormat.format(endDate11)+" To "+dateFormat.format(endDate1), pFontHeader);
                       ...
                       // Phrase phrase = new Phrase("Patients List As At:" +endDate, pFontHeader);
                        ...
                        try {
                           ...
                           // java.sql.ResultSet rset = st.executeQuery("select date,CASE WHEN (activity_code IS NULL) THEN '-' ELSE activity_code END AS activity_code ,CASE WHEN (transaction_no IS NULL) THEN '-' ELSE transaction_no END AS transaction_no,description,transaction_type,sum(debit),sum(credit) from transaction_list_view WHERE date BETWEEN '"+beginDate+"' AND '"+endDate+"' GROUP BY date,activity_code,description,transaction_no,transaction_type ORDER BY transaction_no ASC");// tn,debit_note db WHERE tn.policy_no != '' and tn.policy_no = db.policy_no GROUP BY tn.policy_no,db.policy_class");
                            java.sql.ResultSet rset = st.executeQuery("select input_date,activity_code ,journal_no,description,debit,credit from ac_journal WHERE input_date BETWEEN '"+beginDate+"' AND '"+endDate+"' and journal_no not ilike 'VC%' ORDER BY input_date ASC");// tn,debit_note db WHERE tn.policy_no != '' and tn.policy_no = db.policy_no GROUP BY tn.policy_no,db.policy_class");
