...
public class ProvisionPerSchmPdf implements java.lang.Runnable {
    ...
    public void ProvisionPerSchmPdf(java.sql.Connection connDb, java.util.Date begindate, java.util.Date endate,java.lang.String combox1,java.lang.String combox) {
                ...
                */
                  ...
                  */
                    ...
                    try {
                        ...
                        int headerwidths[] = {12,10,28,17,10,14,14,15};
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset1 = st1.executeQuery("select date,admission_no, item,invoice_no,debit,balance,prov_amt from ac_debtors where account_no = '"+memNo+"' AND prov_date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and provision = true order by date");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                           //   java.sql.ResultSet rsetTotals2 = st22.executeQuery("select sum(balance -credit_bal) from ac_debtors where account_no = '"+memNo+"' AND date::date < '"+beginDate+"' and provision = true");
                            ...
                            java.sql.ResultSet rsetTotals = st2.executeQuery("select sum(debit),sum(credit) from ac_debtors where account_no = '"+memNo+"' AND prov_date BETWEEN '"+beginDate+"' AND '"+endDate+"' and provision = true");
