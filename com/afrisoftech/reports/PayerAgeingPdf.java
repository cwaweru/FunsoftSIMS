...
public class PayerAgeingPdf implements java.lang.Runnable {
    double Credit = 0.00;
    // double current = 0.00;
    //double over30 = 0.00;
    // double over60 = 0.00;
    ...
    public void PayerAgeingPdf(java.sql.Connection connDb,java.util.Date begindate,java.util.Date endate,java.lang.String combox) {
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    com.lowagie.text.HeaderFooter footer = new com.lowagie.text.HeaderFooter(new Phrase(Debtor +": Ageing  Page: ",pFontHeader), true);// FontFactory.getFont(com.lowagie.text.FontFactory.HELVETICA, 12, Font.BOLDITALIC,java.awt.Color.blue));
                    ...
                    try {
                        ...
                        try {
                            ...
                            phrase = new Phrase(Debtor + ": Payer  Ageing as at " +dateFormat.format(endDate2), pFontHeader);
                        ...
                        try {
                            ...
                            for (int i = 0; i < listofAct.length; i++) {
                                ...
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                java.sql.PreparedStatement pset10 = connectDB.prepareStatement("select account_no||' '||scheme_name from ac_schemes WHERE account_no = ?");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                                ...
                                java.sql.PreparedStatement pset1 = connectDB.prepareStatement("select sum(balance) from ac_debtors WHERE account_no = ? and dealer = '"+Debtor+"'  AND date < '"+ageingDates[0][0]+"'");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                                //java.sql.PreparedStatement pset1 = connectDB.prepareStatement("select sum(balance) from ac_debtors WHERE account_no = ? and dealer = '"+Debtor+"'  AND date < '"+ageingDates[ageingDates.length - 1][0]+"'");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                             
                                pset1.setString(1,listofAct[i].toString());
                                java.sql.ResultSet rset1 = pset1.executeQuery();
                                
                                java.sql.PreparedStatement pset111 = connectDB.prepareStatement("select sum(balance-credit_bal) from ac_debtors WHERE account_no = ? and dealer = '"+Debtor+"'  AND date < '"+endDate+"'");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                                ...
                                java.sql.PreparedStatement pset112 = connectDB.prepareStatement("select sum(credit_bal) from ac_debtors WHERE account_no = ? and dealer = '"+Debtor+"' AND date < '"+endDate+"'");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                               ...
                               // for (int t = 0; t < ageingDates.length; t++) {
                                    ...
                                    java.sql.PreparedStatement pset = connectDB.prepareStatement("select sum(balance) from ac_debtors WHERE account_no = ? and dealer = '"+Debtor+"'  AND date between '"+ageingDates[t][0]+"' AND '"+ageingDates[t][1]+"'");
    ...
    public java.lang.Object[] getListofActivities() {
        ...
        try {
            ...
            java.sql.PreparedStatement pSet1 = connectDB.prepareStatement("SELECT DISTINCT account_no FROM ac_debtors where dealer = '"+Debtor+"' and account_no IS NOT NULL and account_no != '' and balance !=0 order by account_no");
