...
public class FinalInPatientlnvSummPdf implements java.lang.Runnable {
    ...
    public void FinalInPatientInvSummPdf(java.sql.Connection connDb, java.lang.String combox) {
        MNo = combox;
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset1 = st1.executeQuery(" select initcap(main_service) as service,sum(dosage),sum(debit-credit) from hp_patient_card where invoice_no = '"+memNo+"' and paid = 'true' and service != 'N.H.I.F' AND service != 'N.H.I.F' AND service not ilike 'Receipt%' AND service not ilike 'Receipt Adj' AND service not ilike 'Discou%' AND service != 'Invoice' group by initcap(main_service) order by service");// union select date::date,initcap(service) as service,dosage,reference,credit from hp_patient_card where patient_no = '"+memNo+"' and credit > 0 order by date");
                            ...
                            java.sql.ResultSet rset11 = st11.executeQuery(" select initcap(main_service) as service,sum(dosage),sum(credit) from hp_patient_card where invoice_no = '"+memNo+"' AND (service = 'N.H.I.F' or service ilike 'Receipt' or service ilike 'Receipt Adj'  OR service ilike 'Discou%') group by main_service order by main_service");// union select date::date,initcap(service) as service,dosage,reference,credit from hp_patient_card where patient_no = '"+memNo+"' and credit > 0 order by date");
