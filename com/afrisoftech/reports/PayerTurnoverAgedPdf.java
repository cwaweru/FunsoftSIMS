...
public class PayerTurnoverAgedPdf implements java.lang.Runnable {
    double Credit = 0.00;
   // double current = 0.00;
    //double over30 = 0.00;
   // double over60 = 0.00;
  //  double over90 = 0.00;
   // double turnOver = 0.00;
    ...
    public void PayerTurnoverAgedPdf(java.sql.Connection connDb,java.util.Date begindate,java.util.Date endate) {
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    com.lowagie.text.HeaderFooter footer = new com.lowagie.text.HeaderFooter(new Phrase("Payer Turnover  Page: ",pFontHeader), true);// FontFactory.getFont(com.lowagie.text.FontFactory.HELVETICA, 12, Font.BOLDITALIC,java.awt.Color.blue));
...
//                            java.util.Date endDate1 = dateFormat.parse(beginDate.toLocaleString());//dateInstance.toLocaleString());
                          ...
                          //  System.out.println(""+endDate1);
                            ...
                            phrase = new Phrase("Payer Turnover Aged as at " +dateFormat.format(endDate2), pFontHeader);
                        ...
                        try {
                            ...
                            double TotalCount = 0.00;   
                               ...
                               while (rset10.next())
                                ...
                                java.sql.PreparedStatement pset1 = connectDB.prepareStatement("select sum(debit) from ac_debtors WHERE dealer = ?  AND date < '"+rangeDates[rangeDates.length - 1][0]+"' and date >='"+beginDate+"' ");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                                pset1.setString(1,listofAct[i].toString());
                                java.sql.ResultSet rset1 = pset1.executeQuery();
                                 
                                java.sql.PreparedStatement pset111 = connectDB.prepareStatement("select sum(debit) from ac_debtors WHERE dealer = ?  AND date between '"+beginDate+"' and '"+endDate+"'");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                                ...
                                */
                                java.sql.PreparedStatement pset112 = connectDB.prepareStatement("select sum(credit) from ac_debtors WHERE dealer = ? AND date between '"+beginDate+"' and '"+endDate+"'");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                                ...
                                for (int t = 0; t < ageingDates.length; t++) {
                                    ...
                                    */
                                    java.sql.PreparedStatement pset = connectDB.prepareStatement("select sum(debit) from ac_debtors WHERE dealer = ?  AND date between '"+rangeDates[t][1]+"' AND '"+rangeDates[t][0]+"'");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
