...
public class DocEarningsDeductionsPdf implements java.lang.Runnable {
    ...
    public void DocEarningsDeductionsPdf(java.sql.Connection connDb, java.util.Date begindate, java.util.Date endate) {
        //MNo = combox;
    ...
    public void generatePdf(java.lang.String memNo) {
            ...
            double osBalancebf = 0.00;
              double Gross = 0.00;      
                    try {
                        ...
                        int headerwidths[] = {13,10,10,10,10,10,10,9,9,9,10};
                        ...
                        table.getDefaultCell().setBorderColor(java.awt.Color.BLACK);
                           ...
                           */
                            ...
                            //  java.sql.ResultSet rset = st.executeQuery("select DISTINCT member_code, member_name,date from shares_transactions order by member_code");
                              java.sql.ResultSet rset1 = st1.executeQuery("select account_no,initcap(doc_name),sum(debit) from ac_doctors_payments where input_date BETWEEN '"+beginDate+"' AND '"+endDate+"' and account_no = '"+listofAct[i]+"' GROUP BY account_no,doc_name order by account_no");// union select date::date,initcap(service) as service,dosage,reference,credit from hp_patient_card where patient_no = '"+memNo+"' and credit > 0 order by date");
                           java.sql.ResultSet rset11 = st11.executeQuery("select sum(credit) from ac_doctors_payments where input_date BETWEEN '"+beginDate+"' AND '"+endDate+"' and description ilike 'cons%' and account_no = '"+listofAct[i]+"'");// union select date::date,initcap(service) as service,dosage,reference,credit from hp_patient_card where patient_no = '"+memNo+"' and credit > 0 order by date");
                          java.sql.ResultSet rset12 = st12.executeQuery("select sum(credit) from ac_doctors_payments where input_date BETWEEN '"+beginDate+"' AND '"+endDate+"'  and description ilike 'debt%' and account_no = '"+listofAct[i]+"'");
                          java.sql.ResultSet rset13 = st13.executeQuery("select sum(credit) from ac_doctors_payments where input_date BETWEEN '"+beginDate+"' AND '"+endDate+"'  and description ilike 'med%' and account_no = '"+listofAct[i]+"'");
                          java.sql.ResultSet rset14 = st14.executeQuery("select sum(credit) from ac_doctors_payments where  input_date BETWEEN '"+beginDate+"' AND '"+endDate+"'  and description ilike 'rent%' and account_no = '"+listofAct[i]+"'");
                            ...
                            while (rset1.next())
                             ...
                             Gross = Gross + rset1.getDouble(3);
                               ...
                               phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(rset13.getString(1)),pFontHeader);
                                ...
                                osBalance2 = osBalance2 + rset14.getDouble(1);
                                table.addCell(phrase);
        ...
        try {
           ...
           // java.sql.PreparedStatement pSet1 = connectDB.prepareStatement("SELECT DISTINCT dealer FROM ac_debtors where date BETWEEN ? AND ? and dealer IS NOT NULL order by dealer");
            java.sql.PreparedStatement pSet1 = connectDB.prepareStatement("SELECT DISTINCT account_no FROM ac_doctors_payments where input_date BETWEEN '"+beginDate+"' AND '"+endDate+"' order by account_no");
