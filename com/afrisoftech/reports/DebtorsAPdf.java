...
public class DebtorsAPdf implements java.lang.Runnable {
                ...
                try {
                   ...
                   */
                     ...
                     */
                        ...
                        //      table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                        phrase = new Phrase("OS Balance",pFontHeader);
                        table.addCell(phrase);
                        /*
                         *
                         *
                        ...
                        try {
                            ...
                            for (int i = 0; i < listofAct.length; i++) {
                                ...
                                java.sql.PreparedStatement pSeta1 = connectDB.prepareStatement("SELECT count(payee) FROM ac_debtors where account_no IS NOT NULL and account_no !='' and balance > 0");
                                ...
                                java.sql.PreparedStatement pset1 = connectDB.prepareStatement("select sum(balance-credit_bal) from ac_debtors WHERE account_no = ? AND dealer IS NOT NULL AND date < '"+beginDate+"'");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                                ...
                                for (int t = 0; t < rangeDates.length; t++) {
                                    ...
                                    java.sql.ResultSet rset01 = st01.executeQuery("select count(balance-credit_bal) from ac_debtors WHERE account_no = '"+listofAct[i]+"' AND dealer IS NOT NULL AND date between '"+rangeDates[t][1]+"' AND '"+rangeDates[t][0]+"'");
                                    
                                    java.sql.PreparedStatement pset = connectDB.prepareStatement("select sum(balance-credit_bal) from ac_debtors WHERE account_no = ? AND dealer IS NOT NULL AND date between '"+rangeDates[t][1]+"' AND '"+rangeDates[t][0]+"'");//< '"+endDate+"'::date and date > '"+endDate+"'::date - 30 group by dealer");
                                    while (rset01.next()) {
