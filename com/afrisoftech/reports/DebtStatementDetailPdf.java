...
public class DebtStatementDetailPdf implements java.lang.Runnable {
    ...
    public void DebtStatementDetailPdf(java.sql.Connection connDb, java.util.Date begindate, java.util.Date endate,java.lang.String combox) {
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            phrase = new Phrase(memNo+ " Statement", pFontHeader11);
                            table.addCell(phrase);
                            ...
                            for (int j = 0; j < listofStaffNos.length; j++) {
                                ...
                                java.sql.ResultSet rset1 = st1.executeQuery("select date,admission_no||''||receipt_no, item,invoice_no,debit,credit,credit_bal,balance from ac_debtors where dealer ilike '"+memNo+"' and payee = '"+listofStaffNos[j]+"' AND date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' order by date");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                                java.sql.ResultSet rsetTotals2 = st22.executeQuery("select sum(balance -credit_bal) from ac_debtors where dealer ilike '"+memNo+"%' AND date::date <'"+beginDate+"' and payee ilike '"+listofStaffNos[j]+"'");
