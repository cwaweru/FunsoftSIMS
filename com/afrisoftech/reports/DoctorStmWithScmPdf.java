...
public class DoctorStmWithScmPdf implements java.lang.Runnable {
    ...
    public void DoctorStmWithScmPdf(java.sql.Connection connDb, java.lang.String combox,java.util.Date begindate, java.util.Date endate) {
        MNo = combox;
        ...
        try {
            ...
            try {
                ...
                try {
                  ...
                  */
                    com.lowagie.text.HeaderFooter footer = new com.lowagie.text.HeaderFooter(new Phrase("Doctor's Statement (all transactions)- Page: "), true);// FontFactory.getFont(com.lowagie.text.FontFactory.HELVETICA, 12, Font.BOLDITALIC,java.awt.Color.blue));
            ...
            double osBalancebf = 0.00;
                    ...
                    try {
                        ...
                        int headerwidths[] = {12,30,8,20,10,9,10,10,15};
                        ...
                        try {
                            ...
                            //java.sql.ResultSet rset1 = st1.executeQuery("select input_date::date,INITCAP(patient_no||' '||patient_name),INITCAP(description),debit,credit,debit_bal,credit_bal,pat_inv from ac_doctors_ledger where acc_no = '"+memNo+"'and input_date BETWEEN '"+beginDate+"' AND '"+endDate+"' order by input_date::date");// union select date::date,initcap(service) as service,dosage,reference,credit from hp_patient_card where patient_no = '"+memNo+"' and credit > 0 order by date");
                          java.sql.ResultSet rset1 = st1.executeQuery("select dl.input_date::date,INITCAP(dl.patient_no||' '||dl.patient_name),INITCAP(dt.payee),dl.debit,dl.credit,dl.debit_bal,dl.credit_bal,dl.pat_inv from ac_doctors_ledger dl ,ac_debtors dt where dl.acc_no = '"+memNo+"'and dl.input_date BETWEEN '"+beginDate+"' AND '"+endDate+"' and dl.pat_inv = dt.invoice_no and dl.patient_no = dt.admission_no order by dl.input_date::date");// union select date::date,initcap(service) as service,dosage,reference,credit from hp_patient_card where patient_no = '"+memNo+"' and credit > 0 order by date");
                            ...
                            phrase = new Phrase("Doctor's Statement (all transactions)", pFontHeader1);
                            table.addCell(phrase);
                            while (rset.next()) 
                            ...
                            phrase = new Phrase("Pat. Inv.",pFontHeader1);
                            table.addCell(phrase);
                           ...
                           // table.addCell(phrase);
                            ...
                            while (rset1.next()){
                                ...
                                phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(java.lang.String.valueOf(osBalance - osBalancebf)), pFontHeader);
