                    ...
                    try {
                       ...
                       table.getDefaultCell().setBorder(Rectangle.BOTTOM);
                        ...
                        try {
                           ...
                           // java.sql.ResultSet rset1 = st1.executeQuery("select al.date,db.admission_no,db.item,al.inv_no,db.debit, al.cheque_no,al.amount,al.amount_alloc,(db.debit-al.amount_alloc) from db_allocation al,ac_debtors db where al.acc_no = '"+memNo+"' and db.invoice_no = al.inv_no AND al.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' order by al.date");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
