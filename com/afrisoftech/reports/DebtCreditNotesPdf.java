...
public class DebtCreditNotesPdf implements java.lang.Runnable {
    ...
    public void DebtCreditNotesPdf(java.sql.Connection connDb,java.util.Date begindate, java.util.Date endate) {
               ...
               /// }
                    try {
                       ...
                       // table.endHeaders();
                        ...
                        int headerwidths[] = {15,8,30,11,15,15,15};
                      ...
                      //  }
                       ...
                       // table.getDefaultCell().setBorderColor(java.awt.Color.WHITE);
                          table.getDefaultCell().setBorderColor(java.awt.Color.BLACK);
                            ...
                            phrase = new Phrase("Credit Notes", pFontHeader11);
                            table.addCell(phrase);
                            ...
                            table.addCell(phrase);
                            table.getDefaultCell().setColspan(1);
                            phrase = new Phrase("CR No.",pFontHeader1);
                            table.addCell(phrase);
                            table.getDefaultCell().setColspan(1);
                        ...
                        try {
                            ...
                            java.sql.Statement st5 = connectDB.createStatement();
                             java.sql.ResultSet rsetTotals2 = st22.executeQuery("select sum(credit_bal) from ac_debtors where date BETWEEN '"+beginDate+"' AND '"+endDate+"'");
                         ...
                         for (int i = 0; i < listofAct.length; i++) {
                           double osBalance1 = 0.00;
                             java.sql.ResultSet rset1 = st1.executeQuery("select date,reference_no, account_no,initcap(payee),sum(debit),sum(balance) from ac_debtors where date BETWEEN '"+beginDate+"' AND '"+endDate+"' and credit > 0 and dealer ilike '"+listofAct[i]+"' group by date,reference_no, account_no,initcap(payee) order by reference_no asc");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
