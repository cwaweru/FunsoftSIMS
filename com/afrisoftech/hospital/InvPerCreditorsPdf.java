...
public class InvPerCreditorsPdf implements java.lang.Runnable {
    ...
    public void InvPerCreditorsPdf(java.sql.Connection connDb, java.util.Date begindate, java.util.Date endate,java.lang.String combox) {
                  ...
                  */
                    ...
                    try {
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset1 = st1.executeQuery("select date,voucher_no, transaction_type,invoice_no,debit,credit from ac_accounts_payable where dealer = '"+memNo+"' AND date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and (transaction_type ILIKE 'Stock Deliverly' OR transaction_type ILIKE 'Raise Invoice' OR transaction_type ILIKE 'INV') order by date");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                       ...
                       //     table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                         ...
                         */
                            ...
                            table.getDefaultCell().setColspan(4);
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_CENTER);
                            phrase = new Phrase("Invoices per Supplier", pFontHeader11);
                            table.addCell(phrase);
