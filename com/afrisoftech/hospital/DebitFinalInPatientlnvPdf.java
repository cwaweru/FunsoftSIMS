...
public class DebitFinalInPatientlnvPdf implements java.lang.Runnable {
    ...
    public void DebitFinalInPatientlnvPdf(java.sql.Connection connDb, java.lang.String combox) {
        MNo = combox;
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            table1.getDefaultCell().setColspan(6);
                            table1.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_CENTER);
                            phrase = new Phrase("DEBIT NOTE", pFontHeader1);
                            table1.addCell(phrase);
                            table1.getDefaultCell().setBorder(Rectangle.BOTTOM);
                            table1.getDefaultCell().setBorderColor(java.awt.Color.WHITE);
                    ...
                    try {
                        ...
                        try {
                            ...
                            //java.sql.ResultSet rset1 = st1.executeQuery(" select date::date,initcap(service) as service,dosage,invoice_no,credit-debit from hp_patient_card where reference = '"+memNo+"' and paid = 'true' and service != 'N.H.I.F' AND service not ilike 'Receipt' AND service != 'Invoice' order by date::date");// union select date::date,initcap(service) as service,dosage,reference,credit from hp_patient_card where patient_no = '"+memNo+"' and credit > 0 order by date");
                             java.sql.ResultSet rset1 = st1.executeQuery(" select date::date,initcap(service) as service,dosage,invoice_no,debit from hp_patient_card where reference = '"+memNo+"' and paid = 'true' AND service != 'Invoice' order by date::date");// union select date::date,initcap(service) as service,dosage,reference,credit from hp_patient_card where patient_no = '"+memNo+"' and credit > 0 order by date");
                       
                            java.sql.ResultSet rsetTotals = st2.executeQuery("select sum(debit) from hp_patient_card where reference = '"+memNo+"' and service != 'Invoice'");
                            
                            java.sql.ResultSet rset11 = st11.executeQuery(" select date::date,initcap(service) as service,dosage,invoice_no,credit from hp_patient_card where reference = '"+memNo+"' order by date::date");// union select date::date,initcap(service) as service,dosage,reference,credit from hp_patient_card where patient_no = '"+memNo+"' and credit > 0 order by date");
                            java.sql.ResultSet rsetTotals1 = st21.executeQuery("select sum(debit) from hp_patient_card where reference = '"+memNo+"'");
