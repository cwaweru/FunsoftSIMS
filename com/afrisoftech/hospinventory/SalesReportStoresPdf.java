...
public class SalesReportStoresPdf implements java.lang.Runnable {
    ...
    public void SalesReportStoresPdf(java.sql.Connection connDb,java.util.Date begindate, java.util.Date endate,java.lang.String receipt) {
        Receipt = receipt;
        connectDB = connDb;
        beginDate = begindate;
        endDate = endate;
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    com.lowagie.text.HeaderFooter footer = new com.lowagie.text.HeaderFooter(new Phrase("Sales Report "+Receipt + "- Page: ",pFontHeader), true);// FontFactory.getFont(com.lowagie.text.FontFactory.HELVETICA, 12, Font.BOLDITALIC,java.awt.Color.blue));
                    ...
                    try {
                        ...
                        try {
                            ...
                            phrase = new Phrase("Sales " +Receipt +" Report :     Period : From "  +dateFormat.format(endDate11)+" To "+dateFormat.format(endDate1), pFontHeader);
                        ...
                        try {
                            ...
                            //java.sql.ResultSet rsetTotals = st.executeQuery("select sum((ph.quantity * st.transfer_price)::numeric(10,2)), sum(amount)::numeric(10,2),sum(amount - ph.quantity * st.transfer_price)::numeric(10,2) from stores_sales_view ph,st_stock_prices st WHERE st.department = ph.revenue_code AND ph.trans_date BETWEEN '"+beginDate+"' AND '"+endDate+"' AND ph.service = st.product");
                            ...
                            for (int i = 0; i < listofAct.length; i++) {
                               ...
                               // java.sql.PreparedStatement st1 = connectDB.prepareStatement("select ph.service,sum(ph.quantity),st.transfer_price::numeric(10,2),sum((ph.quantity * st.transfer_price)::numeric(10,2)), st.selling_price::numeric(10,2),sum(amount)::numeric(10,2),sum(amount - ph.quantity * st.transfer_price)::numeric(10,2) from stores_sales_view ph,st_stock_prices st WHERE ph.service ilike ? AND ph.trans_date BETWEEN '"+beginDate+"' AND '"+endDate+"' AND ph.service ilike st.product and  st.department ilike ph.revenue_code and ph.amount >0 GROUP BY ph.service,st.selling_price,st.transfer_price");
                                //java.sql.PreparedStatement st1 = connectDB.prepareStatement("select ph.item,sum(ph.sub_store_issiuing),st.transfer_price::numeric(10,2),sum((ph.sub_store_issiuing * st.transfer_price)::numeric(10,2)), st.selling_price::numeric(10,2),sum(ph.total)::numeric(10,2),sum(ph.sub_store_issiuing * st.transfer_price)::numeric(10,2) from st_sub_stores ph,st_stock_prices st WHERE ph.item ilike ? AND ph.trans_date BETWEEN '"+beginDate+"' AND '"+endDate+"' AND ph.item ilike st.product and  ph.sub_store ilike '"+Receipt+"' and ph.total >0 and st.department ilike '"+Receipt+"' GROUP BY ph.item,st.selling_price,st.transfer_price");
                                 java.sql.PreparedStatement st1 = connectDB.prepareStatement("select ph.description,sum(ph.quantity) as quantity,st.transfer_price::numeric(10,2) as bprice,sum((ph.quantity * st.transfer_price)::numeric(10,2)) as cost, st.selling_price::numeric(10,2)as sprice,sum(st.selling_price *ph.quantity)::numeric(10,2) as amt from hp_pharmacy ph,st_stock_prices st WHERE ph.main_service ilike '"+Receipt+"' and ph.description = ? AND st.department ilike '"+Receipt+"' AND ph.date_prescribed BETWEEN '"+beginDate+"' AND '"+endDate+"' AND ph.description = st.product GROUP BY ph.description,st.selling_price,st.transfer_price");
                                ...
                                while (rset.next()) {
                                   ...
                                   // phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(rset.getString(7)),pFontHeader1);
                                    ...
                                    osBalance31 = osBalance21-osBalance11;
                                   // osBalance3 = osBalance2-;
                                   
                                     phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(java.lang.String.valueOf(osBalance21-osBalance11)),pFontHeader1);
                                      
                                    table.addCell(phrase);
                                    table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_RIGHT);
                                    
                                    percentage = osBalance31*100/osBalance11;
                                    if ( percentage > 0 || percentage <10000000){
                                        phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(java.lang.String.valueOf(percentage)),pFontHeader1);
                                        //osBalance3 = osBalance3 + rset.getDouble(7);
                                        table.addCell(phrase);
                                    }else{
    ...
    public java.lang.Object[] getListofActivities() {
        ...
        try {
            ...
            java.sql.ResultSet rSet1 = stmt1.executeQuery("SELECT DISTINCT description FROM hp_pharmacy where quantity > 0 and main_service ilike '"+Receipt+"%' and date_prescribed between '"+beginDate+"' and '"+endDate+"'  order by description");
