    ...
    public void generatePdf() {
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            phrase = new Phrase(StoreName+ " Stock Audit Report :     Period : From "  +dateFormat.format(endDate11)+" To "+dateFormat.format(endDate1), pFontHeader);
                        ...
                        //    java.sql.ResultSet rsetTotals = st.executeQuery("select sum((ph.quantity * st.transfer_price)::numeric(10,2)), sum(amount)::numeric(10,2),sum(amount - ph.quantity * st.transfer_price)::numeric(10,2) from hp_pharmacy ph,st_stock_prices st WHERE st.department = 'Pharmacy' AND ph.date_prescribed BETWEEN '"+beginDate+"' AND '"+endDate+"' AND ph.description = st.product");
                            ...
                            for (int i = 0; i < listofAct.length; i++) {
                               ...
                               */
                                ...
                                java.sql.ResultSet rset = st21.executeQuery("select ss.item,ROUND(sum(ss.issuing/sp.packaging),0) from st_sub_stores ss,st_stock_item sp WHERE ss.trans_date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' AND ss.item = '"+listofAct[i]+"' AND ss.store_name = '"+StoreName+"' and ss.item = sp.description group by ss.item");
                                
                                java.sql.ResultSet rset1 = st2.executeQuery("select ROUND(sum(ss.receiving/sp.packaging),0) as receiving from st_sub_stores ss,st_stock_item sp WHERE ss.trans_date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' AND ss.item = '"+listofAct[i]+"' AND ss.store_name = '"+StoreName+"' and ss.item = sp.description group by ss.item");
                                
                                java.sql.ResultSet rset2 = st3.executeQuery("select SUM(quantity_instock) from st_stock_item WHERE description = '"+listofAct[i]+"' AND department = '"+StoreName+"'");
        ...
        public java.lang.Object[] getListofActivities() {
            ...
            try {
                ...
                java.sql.ResultSet rSet1 = stmt1.executeQuery("SELECT DISTINCT item FROM st_sub_stores where trans_date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' AND store_name ='"+StoreName+"' ORDER BY item");
