            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        phrase = new Phrase("Transt No. : "+Receipt , pFontHeader);
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset2 = st2.executeQuery("select distinct store_name,sub_store,issiued_to,user_name from st_sub_stores WHERE transaction_no = '"+Receipt+"' AND issuing > 0");
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset = st.executeQuery("select initcap(ss.item),sum(ss.issuing),sum(ss.price),sum(ss.total) from st_sub_stores ss WHERE ss.transaction_no = '"+Receipt+"' AND ss.issuing > 0 group by ss.item ORDER BY ss.item");
                            //  java.sql.ResultSet rsetTotals = st2.executeQuery("SELECT SUM(net_value) from orders WHERE received = false");// WHERE date BETWEEN '"+beginDate+"' AND '"+endDate+"'");
                            java.sql.Statement stt = connectDB.createStatement();
                          ...
                          */
                            ...
                            while (rset22.next()) {
                                table.getDefaultCell().setColspan(2);
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase("Issued By : "+dbObject.getDBObject(rset22.getObject(4), "-"), pFontHeader);
                               // table.addCell(phrase);
