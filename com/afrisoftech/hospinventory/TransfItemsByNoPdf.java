...
public class TransfItemsByNoPdf implements java.lang.Runnable {
    ...
    public void TransfItemsByNoPdf(java.sql.Connection connDb, java.lang.String receipt, java.lang.String receipt1) {
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        Phrase phrase = new Phrase("Stocks Transfer " , pFontHeader);
                        
                        table.addCell(phrase);
                        
                        phrase = new Phrase("Transfer  No. : "+listofStaffNos[j] , pFontHeader);
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset2 = st2.executeQuery("select distinct store_name,units,sub_store,issiued_to,user_name from st_sub_stores WHERE transaction_no = '"+listofStaffNos[j]+"' AND (issuing > 0 or sub_store_issiuing > 0)");
                            ...
                            table.getDefaultCell().setColspan(3);
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                            phrase = new Phrase("Destination : "+dbObject.getDBObject(rset2.getObject(3), "-"), pFontHeader);
                       ...
                       //for (int j = 0; j < listofStaffNos.length; j++) {
                            ...
                            java.sql.ResultSet rset22 = st22.executeQuery("select distinct store_name,sub_store,issiued_to,user_name from st_sub_stores WHERE transaction_no = '"+listofStaffNos[j]+"' AND (issuing > 0 or sub_store_issiuing > 0)");
                            
                            
                            java.sql.ResultSet rset2 = st2.executeQuery("select distinct store_name,sub_store,issiued_to,user_name from st_sub_stores WHERE transaction_no = '"+listofStaffNos[j]+"' AND (issuing > 0 or sub_store_issiuing > 0)");
                            
                            
                            java.sql.ResultSet rset = st.executeQuery("select initcap(ss.item),ss.units,round(sum(ss.issuing)),sum(ss.price),round(sum(ss.total)) from st_sub_stores ss WHERE ss.transaction_no = '"+listofStaffNos[j]+"' AND ss.store_name Ilike 'main store%' group by ss.item,ss.units ORDER BY ss.item");
                            //  java.sql.ResultSet rsetTotals = st2.executeQuery("SELECT SUM(net_value) from orders WHERE received = false");// WHERE date BETWEEN '"+beginDate+"' AND '"+endDate+"'");
                            java.sql.Statement stt = connectDB.createStatement();
