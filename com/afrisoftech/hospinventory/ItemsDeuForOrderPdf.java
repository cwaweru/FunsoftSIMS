...
public class ItemsDeuForOrderPdf implements java.lang.Runnable {
    ...
    public void ItemsDeuForOrderPdf(java.sql.Connection connDb) {
                    ...
                    try {
                        ...
                        */
                        ...
                        try {
                            ...
                            //java.sql.ResultSet rset11 = st.executeQuery("select round(sum(sc.issuing/(st_ordering_constants.average_day)*st.buying_price)*365*1.33,2) from st_sub_stores sc, st_stock_item st where sc.item = st.description and sc.issuing >0 and sc.trans_date > current_date - (st_ordering_constants.average_day)::integer and st.department ilike 'main store%'");
                            //  while(rset11.next())
                            //Total = rset11.getDouble(1);
                            
                            
                            java.sql.ResultSet rset = st.executeQuery("select description,units,bp,balance,issues,qty,exp_order ,(exp_order*bp) from items_consumption order by description");
