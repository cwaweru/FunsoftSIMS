...
public class EarningBySourcePdf implements java.lang.Runnable {
   ...
   //   com.lowagie.text.ParagraphFont pgraph = Paragraph();
    ...
    public void EarningBySourcePdf(java.sql.Connection connDb, java.util.Date begindate, java.util.Date endate, java.lang.String comboBox) {
         reason = comboBox;
        connectDB = connDb;
        beginDate = begindate;
        endDate = endate;
            ...
            try {
                ...
                try {
                   ...
                   try {
                        ...
                        int headerwidths[] = {13,20,15,22,15,15};
                        ...
                        Phrase phrase = new Phrase("Sacco Earnings From  "  +reason+ " Between "  +dateFormat.format(beginDate)+" and "+dateFormat.format(endDate), pFontHeader2);
                      ...
                      //  table.getDefaultCell().setBorderColor(java.awt.Color.WHITE);
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset = stk.executeQuery("select payroll_no,initcap(member_name) as name, date from sacco_earnings where date BETWEEN '"+beginDate+"' AND '"+endDate+"' and trans_type ilike '"+reason+"' and credit > 0 order by payroll_no");
                            java.sql.ResultSet rset1 = st1.executeQuery("select date, initcap(member_name), payroll_no, initcap(earning_source),sum(credit)- sum(debit) from sacco_earnings where date BETWEEN '"+beginDate+"' AND '"+endDate+"' and trans_type ilike '"+reason+"'and credit > 0 group by date, member_name, payroll_no, earning_source order by date");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                        ...
                        //    table.getDefaultCell().setColspan(7);
                            ...
                            table.getDefaultCell().setColspan(1);
                            phrase = new Phrase("Transaction Type",pFontHeader1);
