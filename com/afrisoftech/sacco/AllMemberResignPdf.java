...
public class AllMemberResignPdf implements java.lang.Runnable {
   ...
   //   com.lowagie.text.ParagraphFont pgraph = Paragraph();
    ...
    public void AllMemberResignPdf(java.sql.Connection connDb, java.lang.String begindate, java.lang.String endate) {
        connectDB = connDb;
        beginDate = begindate;
        endDate = endate;
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        int headerwidths[] = {10,15,9,12,11,11,11,11,10};
                        ...
                        Phrase phrase = new Phrase("All Resigned Members Detailed Statement" , pFontHeader2);
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset = stk.executeQuery("select payroll_no,initcap(member_name) as name, trans_no from member_resignation where date BETWEEN '"+beginDate+"' AND '"+endDate+"' order by date");
                            java.sql.ResultSet rset1 = st1.executeQuery("select date, initcap(member_name), payroll_no, initcap(reason),amount_payable,commission,loan_bal, amount_paid from member_resignation where date BETWEEN '"+beginDate+"' AND '"+endDate+"' order by date");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                            ...
                            table.getDefaultCell().setColspan(1);
                            phrase = new Phrase("Names.",pFontHeader1);
                            table.addCell(phrase);
                           ...
                           table.addCell(phrase);
                            ...
                            table.getDefaultCell().setColspan(1);
                            phrase = new Phrase("Loan Bal. ",pFontHeader1);
                            table.addCell(phrase);
                            ...
                            while (rset1.next()){
                                ...
                                table.addCell(phrase);
                                 ...
                                 Debits = rset1.getDouble(6);
                                TotalDebit = TotalDebit + Debits;
                                
                                 Loan = rset1.getDouble(7);
                            ...
                            table.getDefaultCell().setColspan(1);
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_RIGHT);
                            phrase = new Phrase(new com.afrisoftech.sys.Format2Currency().Format2Currency(java.lang.String.valueOf(TotalLoan)),pFontHeader1);
                            table.addCell(phrase);
