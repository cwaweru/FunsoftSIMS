...
public class ResignedMemberPdf implements java.lang.Runnable {
    ...
    public void ResignedMemberPdf(java.sql.Connection connDb, java.util.Date begindate, java.util.Date endate,java.lang.String combox,java.lang.String combox1) {
            ...
            try {
                ...
                try {
                    ...
                    docPdf.open();
                        ...
                        Phrase phrase = new Phrase("Resigned Member Detailed Statement" , pFontHeader2);
                        ...
                        try {
                            ...
                            java.sql.ResultSet rset = stk.executeQuery("select distinct payroll_no,initcap(member_name) as name, trans_no from member_resignation where payroll_no = '"+tscNo+"'");
                           java.sql.ResultSet rset1 = st1.executeQuery("select date, initcap(reason),amount_payable,commission, amount_paid from member_resignation where payroll_no = '"+tscNo+"' order by date");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                          //  java.sql.ResultSet rset2 = st5.executeQuery("select monthly_repayment,expected_interest from loan_transactions  where loan_account_no = '"+loanNo+"' and date between '"+beginDate+"' and '"+endDate+"' group by monthly_repayment,expected_interest");// UNION select pd.date::date,initcap(pd.scheme_staff_no), (sh.first_name||' '||sh.second_name||' '||sh.last_name) as name,pd.reference,sum(pd.credit),pd.patient_no from hp_patient_card pd,hp_inpatient_register sh where pd.patient_no = sh.patient_no and pd.isurer = '"+memNo+"' AND pd.date::date BETWEEN '"+beginDate+"' AND '"+endDate+"' and pd.credit > 0 group by pd.date::date,pd.scheme_staff_no, name,pd.reference,pd.invoice_no,pd.patient_no order by pd.invoice_no");
                           ...
                           // java.sql.ResultSet rsetTotals2 = st22.executeQuery("select sum(debit-credit) from loan_transactions where loan_account_no = '"+loanNo+"' AND date::date < '"+beginDate+"'");
                            ...
                            while (rset.next()) {
                                ...
                                table.getDefaultCell().setColspan(2);
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase("Resign. No.  "+rset.getObject(3).toString(), pFontHeader1);
                                table.addCell(phrase);
                            }
                            ...
                            table.getDefaultCell().setColspan(1);
                            phrase = new Phrase("Total Shares ",pFontHeader1);
                            table.addCell(phrase);
                            table.getDefaultCell().setColspan(1);
