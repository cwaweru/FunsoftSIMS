...
public class CashReceiptsPdf implements java.lang.Runnable {
    ...
    public void CashReceiptsPdf(java.sql.Connection connDb, java.lang.String combox, java.lang.String amount, java.lang.String receipt, java.lang.String paymode) {
            ...
            try {
                ...
                try {
                    ...
                    */
                    ...
                    try {
                        ...
                        try {
                            ...
                            //  java.sql.ResultSet rsetTotals = st2.executeQuery("select sum(pc.debit - pc.credit) as debit from ac_cash_collection pc where pc.dealer = '"+MNo+"' AND pc.transaction_type = 'Receipts' AND pc.reconciled = false  AND pc.date::date = current_date");
                            //  java.sql.ResultSet rs = st.executeQuery("select cb.receipt_no from ac_cash_collection cb where cb.dealer = '"+MNo+"' AND cb.reconciled = false  AND cb.date::date = current_date");
                            
                            // java.sql.ResultSet rset6 = st6.executeQuery("select CASE WHEN (payment_mode = 'Cash') THEN 'Cash' ELSE card_name||' '||card_no END AS payment_mode, current_user,cash_point from ac_cash_collection where dealer = '"+MNo+"' AND transaction_type = 'Receipts' AND reconciled = false  AND date::date = current_date");
                      ...
                      */
                            ...
                            while (rset1.next()){
                                ...
                                table.addCell(phrase);
                                phrase = new Phrase("Name "+ MNo, pFontHeader);
                            ...
                            table.addCell(phrase);
                            java.sql.ResultSet rset5 = st5.executeQuery("select description,sum(quantity),sum(debit),round(sum(debit/quantity)) from ac_cash_collection where receipt_no = '"+Receipt+"' group by description");
                   ...
                   */
                            ...
                            java.sql.PreparedStatement pstmt251 = connectDB.prepareStatement("UPDATE hp_pharmacy set paid = true where patient_name = '"+MNo+"'");
